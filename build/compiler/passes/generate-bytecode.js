...
function generateBytecode(ast) {
  ...
  function buildCall(functionIndex, delta, env, sp) {
    var params = Object.keys(env).map(function (name) {
      return sp - env[name];
    });
  ...
  var generate = visitor.build({
      ...
      function buildElementsCode(elements, context) {
        if (elements.length > 0) {
          ...
          if (context.action) {
            ...
            return buildSequence([op.LOAD_SAVED_POS, node.elements.length], buildCall(functionIndex, node.elements.length + 1, context.env, context.sp));
          } else {
    ...
    literal: function literal(node) {
      if (node.value.length > 0) {
        var stringIndex = addConst("\"" + js.stringEscape(node.ignoreCase ? node.value.toLowerCase() : node.value) + "\"");
        var expectedIndex = addConst("peg$literalExpectation(" + "\"" + js.stringEscape(node.value) + "\", " + node.ignoreCase + ")");
        ...
        var _stringIndex = addConst("\"\"");

        return [op.PUSH, _stringIndex];
      }
    },
    class: function _class(node) {
      var regexp = "/^[" + (node.inverted ? "^" : "") + node.parts.map(function (part) {
        return Array.isArray(part) ? js.regexpClassEscape(part[0]) + "-" + js.regexpClassEscape(part[1]) : js.regexpClassEscape(part);
      }).join("") + "]/" + (node.ignoreCase ? "i" : "");
      var parts = "[" + node.parts.map(function (part) {
        return Array.isArray(part) ? "[\"" + js.stringEscape(part[0]) + "\", \"" + js.stringEscape(part[1]) + "\"]" : "\"" + js.stringEscape(part) + "\"";
      }).join(", ") + "]";
      var regexpIndex = addConst(regexp);
      var expectedIndex = addConst("peg$classExpectation(" + parts + ", " + node.inverted + ", " + node.ignoreCase + ")");
